...
f) 
The mean of the **bill length** is *`r round(mean(pull(penguins, bill_length_mm), na.rm = TRUE), digits = 2)`*.
The standard deviation of the bill length is *`r round(sd(pull(penguins, bill_length_mm), na.rm = TRUE), digits = 2)`*.

g) 
The mean of the **bill depth** is *`r round(mean(pull(penguins, bill_depth_mm), na.rm = TRUE), digits = 2)`*.
The standard deviation of the bill depth is *`r round(sd(pull(penguins, bill_depth_mm), na.rm = TRUE), digits=2)`*.

h) 
The mean of the **flipper length** is *`r round(mean(pull(penguins, flipper_length_mm),na.rm = TRUE))`*.
The standard deviation of the flipper length is  *`r round(sd(pull(penguins, flipper_length_mm), na.rm = TRUE), digits=2)`*.


i)
The mean of the **body mass** is *`r round(mean(pull(penguins, body_mass_g), na.rm = TRUE), digits=2)`*.
The standard deviation of the body mass is *`r round(sd(pull(penguins, body_mass_g), na.rm = TRUE), digits=2)`*.

j) The number of *rows* in the penguins data set is **`r nrow(penguins)`**.
k) The number of *columns* in the penguins data set is **`r ncol(penguins)`**.
