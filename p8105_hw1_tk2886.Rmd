...
When I take the mean of the logical vector, the mean functions work on this logical vector even though that does not make sense. If someone looks at the data frame they will see that the logical vector in my data frame displays True and False. The True is being converted to 1 and False is being converted to 0. I have four True out of 10 and that is 0.4 which is the same value as the mean function is outputting. 
...
b) The unique **species** in the penguins data table are *`r unique(pull(penguins, species))`*.

c) The unique **island** in the penguins data table are *`r unique(pull(penguins, island))`*.

d) The unique **years** in the penguins data table are *`r unique(pull(penguins, year), na.rm = TRUE)`*.

##e) The **sex** in the penguins data table are *`r unique(na.omit(pull(penguins, sex)))`*.##

f) The mean of the **bill length** is *`r round(mean(pull(penguins, bill_length_mm), na.rm = TRUE), digits = 2)`* and the standard deviation of the bill length is *`r round(sd(pull(penguins, bill_length_mm), na.rm = TRUE), digits = 2)`*.

g) The mean of the **bill depth** is *`r round(mean(pull(penguins, bill_depth_mm), na.rm = TRUE), digits = 2)`* and the standard deviation of the bill depth is *`r round(sd(pull(penguins, bill_depth_mm), na.rm = TRUE), digits=2)`*.

h) The mean of the **body mass** is *`r round(mean(pull(penguins, body_mass_g), na.rm = TRUE), digits=2)`* and the standard deviation of the body mass is *`r round(sd(pull(penguins, body_mass_g), na.rm = TRUE), digits=2)`*.

i) There is *`r nrow(penguins)`* **rows** in this data table. There is *`r ncol(penguins)`* **columns** in this data table. 

j) The mean of the **flipper length** is *`r round(mean(pull(penguins, flipper_length_mm), na.rm = TRUE))`* and the standard deviation of the flipper length is *`r round(sd(pull(penguins, flipper_length_mm), na.rm = TRUE))`*. 
